...
static down_item_program* item_create(GXxmlNodePtr GXxml_node)
{
	...
	while(node)
	{
		...
		else if(0 == GXxmlStrcmp(node->name, STR_CLASS))
		{
			;//item->class = GXxmlNodeGetContent(node);
		}
		else if(0 == GXxmlStrcmp(node->name, STR_LANGUAGE))
		{
			;//item->language = GXxmlNodeGetContent(node);
		}			
		else if(0 == GXxmlStrcmp(node->name, STR_PUBDATE))
		{
			item->pubdate = GXxmlNodeGetContent(node);
		}
		else if(0 == GXxmlStrcmp(node->name, STR_MEDIA))
		{
			;//item->media = GXxmlNodeGetContent(node);
		}
...
static status_t list_create(down_list_program* list, GXxmlNodePtr GXxml_node)
{
	...
	while(node)
	{
		if(0 == GXxmlStrcmp(node->name, STR_VIDEO))
		{
			item_num++;
		}
		node = node->next;
	...
	printf("[DOWN] create program list, from class pc, item num:%d\n", list->item_num);
...
status_t program_parser_pc(const char *filename)
{
	char* buff = NULL;
	int size = 0;
	GXxmlDocPtr doc;
	...
	//buff = net_get((char*)filename);
	if(NULL == buff) return GXCORE_ERROR;

	
	/*parser memory*/
	doc = GXxmlParseMemory(buff, size);
	NET_FREE(buff);
	...
	if(GXxmlStrcmp(root->name, STR_VIDEOLIST))
	{
		GXxmlFreeDoc(doc);
		return GXCORE_ERROR;
	}
	...
	list_create(&object_list_program, root);
