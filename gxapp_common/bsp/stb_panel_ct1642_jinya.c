...
typedef struct GXPANEL_CT1642_Config_s
{
	...
	uint8_t m_chStandbyPin;  //the standby pin of hc164
...
{	
	...
	{	
		...
		case 0:
		case 3:
			if(PANEL_SHOW_OFF == g_PanelCT1642Config.m_chShowMode)
			{
				g_PanelCT1642Config.m_tPanelPublicConfig.m_PanelSetGpioLevelFun(g_PanelCT1642Config.m_chClkPin
											, GXPIO_BIT_LOW_LEVEL);
				g_PanelCT1642Config.m_tPanelPublicConfig.m_PanelSetGpioLevelFun(g_PanelCT1642Config.m_chDataPin
...
{   
	uint8_t chData;
	chData =0x80;
	gx_stb_panel_ct1642_send_data_to_led(chData, g_PanelCT1642Config.m_chCurLenCnt);
}


void gx_stb_panel_ct1642_key_scan(void)
{	
	uint64_t nKeyLevel,nKeyLevel1;
	...
	g_PanelCT1642Config.m_tPanelPublicConfig.m_PanelGetGpioLevelFun(g_PanelCT1642Config.m_chStandbyPin,&nKeyLevel1);

	if(!nKeyLevel1)//如果有按键
	{
		gx_stb_panel_ct1642_send_keymsg(6);
		s_chPress = 1;
	}
...
{	
	...
	if(s_Lockenable)
	{
		g_PanelCT1642Config.m_nLedAddr[0] = 0x7f00;//0x7f00
		g_PanelCT1642Config.m_nLedAddr[1] = 0xdf00;//0xbf00
	}
	else
	{
		g_PanelCT1642Config.m_nLedAddr[0] = 0x7f00;//0x7f00
		g_PanelCT1642Config.m_nLedAddr[1] = 0x7f00;//0xbf00
	}
...
{
	bsp_printf("gx_stb_panel_ct1642_lock\n");
...
void gx_stb_panel_ct1642_standby(void)
{
	...
	gs_chLedValue[0] = gs_chLedData[LED_DATA_8];
}
...
void gx_stb_panel_ct1642_power_off(void)
{
	...
	gx_stb_panel_ct1642_standby();
 	gx_stb_panel_ct1642_send_data_to_led(gs_chLedValue[1], 1);
	gx_stb_panel_ct1642_send_data_to_led(gs_chLedValue[1], 1);
 }
...
void gx_stb_panel_ct1642_init_pin(void)
{
	...
	uint64_t nCfgPin = (((uint64_t)1) << g_PanelCT1642Config.m_chClkPin)
					...
					| (((uint64_t)1) << g_PanelCT1642Config.m_chStandbyPin);
		...
		&& g_PanelCT1642Config.m_nPortBaseAddr[1] == 0)
	{
		...
		g_PanelCT1642Config.m_tPanelPublicConfig.m_PanelOpenGpioFun(g_PanelCT1642Config.m_chStandbyPin, GPIO_INPUT);
...
#if defined(LINUX_OS)
	create_bsp_timer(gx_stb_panel_ct1642_scan_manager,1);
#endif
...
//	g_PanelCT1642Config.m_chClkPin = 23;
//	g_PanelCT1642Config.m_chDataPin = 22;
//	g_PanelCT1642Config.m_chKeyPin[0] = 25;
//	g_PanelCT1642Config.m_chKeyPin[1] = 25;
	g_PanelCT1642Config.m_chClkPin = PANEL_CLK_GPIO;
	g_PanelCT1642Config.m_chDataPin = PANEL_DATA_GPIO;
	g_PanelCT1642Config.m_chKeyPin[0] = 13;
	g_PanelCT1642Config.m_chKeyPin[1] = 13;
	g_PanelCT1642Config.m_chStandbyPin = PANEL_STANDBY_GPIO;

	
	g_PanelCT1642Config.m_chLockPin = 0;
	#ifdef MANUFACTURE_BOSHANG
	...
	g_PanelCT1642Config.m_nLedAddr[0] = 0x7f00;//0x7f00
	g_PanelCT1642Config.m_nLedAddr[1] = 0x7f00;//0xbf00
	g_PanelCT1642Config.m_nLedAddr[2] = 0xbf00;//0xdf00
	g_PanelCT1642Config.m_nLedAddr[3] = 0x7f00;//0xef00	
	g_PanelCT1642Config.m_nLedAddr[4] = 0xff00;
	
	g_PanelCT1642Config.m_chLedCnt = 2;
	g_PanelCT1642Config.m_chCurLenCnt = 0;
...
int gx_stb_panel_ct1642_powerkey_scan(void)
{
	uint64_t nKeyLevel1;
...
GXPANEL_PANEL_ControlBlock_t gs_PanelControlBlock ={
	...
	.m_PanelStandByFun = gx_stb_panel_ct1642_power_off,//gx_stb_panel_ct1642_standby,
